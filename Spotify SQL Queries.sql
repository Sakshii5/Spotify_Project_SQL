...
select name as Artist,round(sum(unit_price*quantity),2) as total_revenue from cte group by name;
...
SELECT title as Albums FROM revenue_cte ORDER BY Total_Revenue DESC LIMIT 5;
...
select name as Genres from genre_revenue_cte order by total_revenue desc limit 5;
...
select c.customer_id,concat(c.first_name,' ',c.last_name) as name, round(sum(i.total),2) as total_purchase from 
...
dense_rank() over(partition by billing_country order by total desc) as rnk from cte) as cte2 where rnk=1;

#8. Popular genre in each city of each country
with cte as (select g.name,i.billing_country,round(sum(i.total),2) as total from genre g inner join track t on g.genre_id=t.genre_id
inner join invoice_line il on t.track_id=il.track_id
inner join invoice i on il.invoice_id=i.invoice_id
group by g.name,i.billing_country)
select name as Genre,billing_country as Country from 
(select * , rank() over(partition by billing_country order by total desc) as rnk from cte ) as cte2 where rnk=1;

#9. Which tracks are the most purchased or streamed?
select t.name as Track_Name, count(t.name) as Total_No_Of_Purchases from track t 
inner join invoice_line il on 
t.track_id=il.track_id 
group by Track_Name order by Total_No_Of_Purchases desc limit 5;

#10.Which employee has generated the highest total invoice amount?
select e.employee_id,e.first_name as name,round(sum(i.total),2) as Invoice_Amount
...
group by e.employee_id,e.first_name order by Invoice_Amount desc limit 1;

#11. What is the total revenue generated by each employee?
select e.employee_id,e.first_name as Employee_Name, round(sum(i.total),2) as Revenue_Generated
from invoice i inner join customer c on i.customer_id=c.customer_id
inner join employee e on c.support_rep_id=e.employee_id
group by e.employee_id,e.first_name order by Revenue_Generated desc;
...
SELECT City,COUNT(Customer_Id) AS CustomerCount 
FROM Customer 
GROUP BY City ORDER BY CustomerCount DESC LIMIT 1;

#14 Which tracks are the most sold, and how many times were they purchased?
SELECT t.Name AS TrackName,COUNT(il.Invoice_Line_Id) AS PurchaseCount 
